...
if(isset($_POST['signup'])){ // checks if the signup button is pressed
  ...
  $verification_code = rand(1000,10000); // create a random 4 digit code 
   ...
   $select = $conn->prepare("SELECT * FROM `service_buyers` WHERE email = ?");
   $select->execute([$email]);

   if($select->rowCount() > 0){
      echo"<script> alert('Email is already in use!'); </script>";
   }else{
        $insert = $conn->prepare("INSERT INTO `service_buyers`(email, password, buyer_name, verification_code) VALUES(?,?,?,?)");
        $insert->execute([$email, $password, $full_name, $verification_code]); // inserts user details into DB

        if($insert){
                $notification_message = "Hooray, Good News! New service buyer has just registered to our website!";
                $notification_type  = "system";
                $send_notification = $conn->prepare("INSERT INTO `notifications` (username, message, type) VALUES(?,?,?)");
                $send_notification->execute([$full_name, $notification_message, $notification_type]);
               ...
               $mail->Body = 'Thank you for registering as a service buyer at LankanServices, We hope you will enjoy your time spent on our platform. <br> 
               ...
               header('location:user_verify.php'); // if success redirects to the verification page
        }
    }
}
?>
...
<head>
    ...
    <title>Signup as a buyer | LankanServices</title>
...
<main class="form-signin w-100 m-auto">
  <form action="" method="post">
    ...
    <h1 class="h3 mb-3 fw-normal">Buyer Signup Form</h1>
    ...
    <p class="mt-5 text-body-secondary">Are you a provider? <a href="provider/signup.php">Click here to register</a></p>
